...
// play it safe and align the stack to 16-bytes
// this is required on 64-bit platforms and also on certain 32-bit platforms
const int stack_align = 16;

static void finish_layout(call_frame_layout *layout) {
  // stack alloc + program counter + frame pointer
  size_t m = (layout->stack_size + sizeof(void*) * 2) % stack_align;
  if (m > 0) {
    layout->stack_size += stack_align - m;
  }
}
